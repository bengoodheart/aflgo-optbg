...
#-------------------------------------------------------------------------------
if [ $RESUME -le $STEP ]; then
  echo "($STEP) Computing dominance for call graph .."

  $AFLGO/patodom.py -d $TMPDIR/dot-files/callgraph.dot \
                    -t $TMPDIR/Ftargets.txt \
                    -b $TMPDIR/FfirstBB.txt \
                    -o $TMPDIR/DomBBcg.txt > $TMPDIR/step${STEP}.log 2>&1 || FAIL=1
